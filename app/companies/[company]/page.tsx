...
export async function generateMetadata({ params }: { params: Promise<{ company: string }> }) {
  ...
  const label = unslugify(company);
  return {
    title: `${label} Jobs | Jobs Home Online`,
    description: `Browse the latest job openings at ${label}. Find remote and on-site roles across engineering, marketing, design, finance and more.`,
    alternates: {
      canonical: `https://jobshomeonline.com/companies/${company}`,
    },
    openGraph: {
      title: `${label} Jobs | Jobs Home Online`,
      description: `Browse the latest job openings at ${label}.`,
      url: `https://jobshomeonline.com/companies/${company}`,
...
export default async function CompanyPage({ params }: { params: Promise<{ company: string }> }) {
  ...
  try {
    ...
    const raw = await Job.find({
      "company.name": { $regex: company.replace(/-/g, " "), $options: "i" },
    })
      .sort({ postedAt: -1 })
      .limit(20)
      .lean();
    ...
    console.error("CompanyPage DB error:", err);
  }

  if (jobs.length === 0) notFound();

  const companyLogo = jobs[0]?.company?.logo;
  const companyName = jobs[0]?.company?.name ?? label;
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="pb-8 border-b border-gray-200 mb-8">
          <div className="flex items-center gap-4 mb-4">
            {companyLogo ? (
              <img
                src={companyLogo}
                alt={companyName}
                className="w-14 h-14 rounded-xl border border-gray-200 object-contain p-1 bg-white"
              />
            ) : (
              <div className="w-14 h-14 rounded-xl border border-gray-200 bg-blue-50 flex items-center justify-center">
          ...
          <p className="mt-3 text-gray-500 text-sm max-w-xl leading-relaxed">
            Browse all open positions at {companyName}. New roles added daily.
          </p>
