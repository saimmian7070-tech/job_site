          ...
          <div className="mt-16 pt-10 border-t border-white/8 grid grid-cols-2 sm:grid-cols-4 gap-8">
            {[
              ...
              <div key={s.label}>
                ...
                <div className="text-xs text-slate-500 mt-1.5 font-medium uppercase tracking-wider">{s.label}</div>
              </div>
            ))}
          </div>
        </div>
          ...
          <div className="min-w-0 space-y-16">
            ...
            {featured && (
              <section>
                ...
                >
                  {/* Accent header strip */}
                  <div className="bg-slate-900 px-6 md:px-8 py-3.5 flex items-center justify-between">
                    <span className="text-[11px] font-bold uppercase tracking-[0.1em] text-slate-400">Editor's Pick</span>
                    <svg className="w-4 h-4 text-slate-500 group-hover:translate-x-1 transition-transform" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                  ...
                  {jobs.map((job) => (
                    ...
                    >
                      ...
                      <div className="flex items-center gap-4 min-w-0">
                        {/* Company avatar */}
                        <div className="w-10 h-10 rounded-xl bg-gray-100 border border-gray-200 flex items-center justify-center shrink-0 font-black text-gray-500 text-sm select-none">
                  ...
                  <div className="grid sm:grid-cols-2 gap-4 mb-4">
                    {latestBlogs.slice(0, 2).map((post) => (
                      <Link
                        key={post._id}
                        href={`/blog/${post.slug ?? "#"}`}
                        className="group flex flex-col bg-white border border-gray-200 rounded-2xl p-6 hover:border-blue-300 hover:shadow-md transition-all"
                      >
                ...
                <div className="grid sm:grid-cols-2 gap-4">
                  {latestBlogs.map((post) => (
                    <Link
                      key={post._id}
                      href={`/blog/${post.slug ?? "#"}`}
                      className="group bg-white border border-gray-200 rounded-2xl p-5 hover:border-blue-300 hover:shadow-sm transition-all block"
                    >
            ...
            {latestBlogs.slice(4, 8).length > 0 && (
              <div className="bg-white border border-gray-200 rounded-2xl p-5 shadow-sm">
                ...
                <div className="space-y-4">
                  {latestBlogs.slice(4, 8).map((post) => (
                    <Link
                      key={post._id}
                      href={`/blog/${post.slug ?? "#"}`}
